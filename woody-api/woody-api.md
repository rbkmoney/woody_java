...
Основан на thread-context подходе (инфомация о цепочке хранится в контексте, который привязан к потоку, в котором началась выполнение или обработка запроса).
...
__Span__ - содежит информацию о текущей цепочке (`trace_id`,  `span_id`, `parent_id`, `duration` и т.д.), а так же `metadata`:
...
-  `metadata` - метаданные запроса, эти данныне не передаются при вызове внешних сервисов. Содержат информацию о вызове, транспорте, ошибках и другую информацию woody-api и его реализации.
...
При разработке сервиса иногда возникает необходимость обработать  запросы в потоке, отличном от того, который принял запрос и владеет информацией о контексте вызова. К примеру сложить их в очередь и обрабатывать в пуле. Для этих целей в woody-api есть соответсвующий пакет:
 `com.rbkmoney.woody.api.concurrent`
 
 С его помощю можно создать объекты `Runnable` и `Callable`, которые будут импользовать указанный конекст, либо текущий контекс потока, если он не указан явно.
...
__Пример создания `WExecutorExecutor`:__
